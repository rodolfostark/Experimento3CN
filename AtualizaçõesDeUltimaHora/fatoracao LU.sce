function [Lf, Uf] = fatoracaoLU(A)//LEMBRAR QUE A DEVE SER QUADRADA!
    ...
    if A(1,1)==0 then 
        ...
        A = perm1(A);//perm(A) é o produto entre a matriz de permutação P e a matriz A


    end
    
    Lf = zeros(size(A)); //Criando minha matriz L completamente zerada, para posteriormente ser preenchida com os fatores.
    
    for i=1:1:numLinhas //vou percorrer apenas a quantidade de elementos da diagonal principal.
        Lf(i,i) = 1;//Aqui eu adiciono os termos 1 da diagonal principal de minha matriz L.
    end
    ...
    for i=1:1:numLinhas-1 //o valor da última linha nunca será pivô para elementos mais abaixo (já que é o último elemento)!
        ...
        for  j=i+1:1:numColunas
            ...
            Lf(j,i) = fator; //aqui atribuo o devido elemento a minha matriz L.
        end
    end
...
function PA = perm1(A) // a matriz PA é o produto entre a matriz A e a matriz de permutação P
    ...
    PA = P*A;
    disp("Cuidado! A=PLU, onde P vale..");
    disp(P);
endfunction
